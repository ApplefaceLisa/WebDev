...
myApp.filter("search", function() {
  ...
  return function(input, searchby, key) {
    ...
    for (var i = 0; i < input.length; i++) {
      if (input[i][searchby].toLowerCase().includes(key.toLowerCase()))
        res.push(input[i]);
    }
    return res;
  }
...
myApp.controller('userCtrl', function($scope, searchFilter) {
  ...
  $scope.searchData = {
    searchOptions: ['fName', 'lName', 'title'],
    searchBy : 'fName',
    searchKey : ''
  }

  $scope.search = function() {
    console.log($scope.searchData.searchKey);
  }
  ...
  $scope.saveChanges = function(id) {
    if (id === "new") {
      ...
      $scope.users.push(new user(id, $scope.fName, $scope.lName, $scope.title, $scope.sex, $scope.age));
    } else {
